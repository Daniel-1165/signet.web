...
export default function ChatSidebar({ onSelectRoom, selectedRoomId }: { onSelectRoom: (id: string) => void, selectedRoomId: string }) {
  ...
  return (
    <div className="w-[340px] flex flex-col h-full bg-[#0a0f0a] border-r border-white/5 pt-32 pb-8 px-6">
      <div className="mb-10">
        <h2 className="text-2xl font-bold tracking-tight text-white mb-6 font-heading">Collective.</h2>
        <div className="relative group">
          <Search size={16} className="absolute left-5 top-1/2 -translate-y-1/2 text-white/20 group-focus-within:text-[#4ade80] transition-colors" />
          <input 
            type="text" 
            placeholder="Search channels..." 
            className="w-full h-12 bg-[#121812] border border-white/5 rounded-2xl pl-12 pr-4 focus:outline-none focus:border-[#4ade80]/20 text-xs tracking-wider transition-all placeholder:text-white/10"
          />
        </div>
      </div>
      ...
      <div className="flex-1 space-y-8">
              ...
              <button
                ...
                className={`flex items-center gap-3 w-full p-4 rounded-2xl transition-all group ${
                  selectedRoomId === room.id 
                    ? 'bg-[#4ade80]/10 border border-[#4ade80]/20 text-white signet-glow' 
                    : 'text-white/40 hover:bg-[#121812] hover:text-white border border-transparent signet-glow-hover'
                }`}
              >
                <div className={`w-8 h-8 rounded-xl flex items-center justify-center transition-all ${
                  selectedRoomId === room.id ? 'bg-[#4ade80] text-[#0a0f0a]' : 'bg-[#1a201a] text-white/20 group-hover:text-[#4ade80]'
                }`}>
        ...
        <div>
          <div className="flex items-center justify-between mb-4 px-2">
            ...
            <span className="px-2 py-0.5 rounded-full bg-[#4ade80]/10 text-[8px] font-bold text-[#4ade80] uppercase tracking-widest animate-pulse border border-[#4ade80]/20">Encrypted</span>
          </div>
      ...
      <div className="mt-auto px-4 py-6 bg-[#121812] rounded-3xl border border-white/5 group hover:border-[#4ade80]/20 transition-all">
        <div className="flex items-center gap-4">
          <div className="w-10 h-10 rounded-xl bg-[#4ade80]/10 border border-[#4ade80]/20 animate-pulse" />
          <div>
          ...
          <MoreHorizontal size={14} className="ml-auto text-white/10" />
        </div>
      </div>
    </div>
  )
