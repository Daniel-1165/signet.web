...
export default function IQTestPage() {
    ...
    if (view === "results" && results) {
        return (
            <div className="min-h-[100dvh] relative overflow-hidden bg-gradient-to-tr from-mist/30 via-white to-white text-ink font-sans px-6 py-20">
                <div className="absolute top-0 right-0 w-[400px] md:w-[600px] h-[400px] md:h-[600px] bg-verdant/5 rounded-full blur-3xl -z-10 translate-x-1/4 -translate-y-1/2"></div>
                                ...
                                {Object.entries(results.domainScores).map(([key, score]) => (
                                    <div key={key} className="flex flex-col">
                                        ...
                                        <div className="w-full h-2 bg-mist border border-mist/30 rounded-full overflow-hidden relative">
                                            <motion.div
                                                className="absolute top-0 left-0 bg-verdant h-full rounded-full"
                                                initial={{ width: 0 }}
                                                animate={{ width: `${score}%` }}
