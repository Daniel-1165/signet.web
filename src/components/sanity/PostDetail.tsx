...
const ptComponents: any = {
  block: {
    ...
    h4: ({ children }: any) => <h4 className="text-xl md:text-2xl font-bold text-[#051F20] mt-6 mb-3">{children}</h4>,
    ...
    <div className="bg-[#FDFCF9] min-h-screen">
      <div className="max-w-[1400px] mx-auto px-6 md:px-12 flex flex-col md:flex-row gap-16 py-12 md:py-24">
        ...
        <aside className="hidden lg:flex flex-col w-48 shrink-0 sticky top-32 h-fit space-y-12">
          ...
          <div className="flex flex-col gap-4">
             ...
             <div className="space-y-4">
                <p className="text-xs text-[#051F20]/60 leading-relaxed font-semibold">
                   Hello! I&apos;m {post.author?.name || 'an editor'}, sharing strategic insights for the Signet network. Learn more <Link href="/resources" className="text-[#8A5A37] hover:underline">about us</Link>.
                </p>
          ...
          <nav className="flex flex-col gap-6">
            {['Articles', 'Books', 'Courses', 'Podcast'].map((item) => (
              <Link 
                key={item} 
                href="/resources" 
                className={`text-sm font-black uppercase tracking-widest transition-colors ${item === 'Articles' ? 'text-[#163832]' : 'text-[#051F20]/30 hover:text-[#163832]'}`}
              >
                {item}
              </Link>
            ))}
          ...
          <Link href="/blog" className="flex items-center gap-2 text-[10px] font-black uppercase tracking-widest text-[#051F20]/40 hover:text-[#051F20] transition-colors pt-12 border-t border-[#051F20]/5">
        ...
        <main className="flex-1 max-w-4xl mx-auto">
          ...
          <header className="mb-12 md:mb-20">
            <div className="flex items-center gap-4 text-[10px] font-black uppercase tracking-[0.2em] text-[#163832] mb-6">
              <span className="px-3 py-1 bg-[#DAF1DE] rounded-full">Intellectual Maturity</span>
              <span>{new Date(post.publishedAt).toLocaleDateString(undefined, { year: 'numeric', month: 'long', day: 'numeric' })}</span>
            </div>
            
            <h1 className="text-3xl sm:text-4xl md:text-5xl font-black text-[#051F20] leading-[1.1] tracking-tighter mb-8 bg-gradient-to-br from-[#051F20] to-[#163832] bg-clip-text text-transparent">
              {post.title}
            </h1>
            ...
            <div className="flex items-center justify-between py-8 border-y border-[#051F20]/5">
               <div className="flex items-center gap-4">
                  ...
                  <div className="flex flex-col">
                    ...
                    <span className="text-sm font-bold text-[#051F20]">{post.author?.name || 'Signet Team'}</span>
                  </div>
               </div>
               <div className="flex items-center gap-4">
                  <button className="p-3 bg-white border border-[#051F20]/5 rounded-full hover:bg-[#FDFCF9] transition-colors"><Share2 size={18} className="text-[#051F20]/60" /></button>
                  <button className="p-3 bg-white border border-[#051F20]/5 rounded-full hover:bg-[#FDFCF9] transition-colors"><Bookmark size={18} className="text-[#051F20]/60" /></button>
               </div>
            </div>
          </header>
          ...
          {/* Hero Image */}
          {post.mainImage && (
            <div className="relative aspect-[16/9] w-full mb-16 md:mb-24 rounded-[2rem] md:rounded-[3rem] overflow-hidden shadow-2xl shadow-[#163832]/5 group">
              <Image
          ...
          <footer className="mt-24 pt-24 border-t border-[#051F20]/10 max-w-3xl mx-auto">
             <div className="p-10 md:p-16 bg-[#FAFAFA] rounded-[3rem] border border-[#051F20]/5 flex flex-col md:flex-row gap-10 items-center md:items-start text-center md:text-left relative overflow-hidden group">
                <div className="absolute top-0 right-0 w-64 h-64 bg-[#DAF1DE]/30 blur-[100px] pointer-events-none group-hover:bg-[#DAF1DE]/50 transition-colors duration-700" />
                ...
                <div className="flex-1 z-10">
                   <span className="text-[10px] font-black uppercase tracking-[0.3em] text-[#163832] mb-4 block">About the Author</span>
                   <h3 className="text-3xl font-bold text-[#051F20] mb-4">{post.author?.name}</h3>
                   <p className="text-lg text-[#051F20]/60 leading-relaxed font-medium mb-6 italic">
                   ...
                   <div className="flex gap-6 justify-center md:justify-start">
                      <Link href="/resources" className="text-xs font-black uppercase tracking-widest text-[#163832] hover:translate-x-1 transition-transform inline-flex items-center gap-2">
                        View all posts <ArrowLeft className="rotate-180 w-3 h-3" />
                      </Link>
                      <div className="flex gap-4 text-[#051F20]/30 ml-auto md:ml-0">
                         <Twitter size={20} className="hover:text-[#163832] cursor-pointer" />
                         <Linkedin size={20} className="hover:text-[#163832] cursor-pointer" />
                      </div>
                   </div>
                </div>
             </div>
        ...
        <aside className="hidden xl:flex flex-col w-64 shrink-0 space-y-12 pt-24 sticky top-32 h-fit">
           <div className="p-8 bg-[#163832] rounded-[2rem] text-white">
              <span className="text-[8px] font-black uppercase tracking-[0.3em] text-white/50 mb-3 block">Network Brief</span>
              ...
              <button className="w-full py-3 bg-white text-[#163832] text-[10px] font-black uppercase tracking-widest rounded-xl hover:bg-[#8EB69B] hover:text-white transition-all">Join Signet</button>
           </div>
           
           <div className="space-y-6">
              <h5 className="text-[10px] font-black uppercase tracking-widest text-[#051F20]/40 border-b border-[#051F20]/5 pb-3">Top Stories</h5>
              <div className="space-y-6">
                 {[1, 2, 3].map(i => (
                    <div key={i} className="group cursor-pointer">
                       <span className="text-[10px] font-bold text-[#163832] block mb-2 opacity-50">0{i}</span>
                       <p className="text-sm font-bold text-[#051F20] group-hover:text-[#163832] transition-colors line-clamp-2 leading-snug">The Architecture of Resilience in Operational Environments</p>
                    </div>
                 ))}
              </div>
           </div>
